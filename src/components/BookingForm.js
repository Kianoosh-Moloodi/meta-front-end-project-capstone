...
// Default times to be displayed in the form
...
const BookingForm = ({ onSubmit }) => {
  const [availableTimes, dispatch] = useReducer(timesReducer, DEFAULT_TIMES);
  ...
  // Fetch available times from API when the date value changes
  useEffect(() => {
    const date = formik.values.date ? new Date(formik.values.date) : new Date();
      ...
      >
        ...
        >
          ...
          {availableTimes.map((time) => (
            <option data-testid='time-select-option' key={time} value={time}>
              {time}
            </option>
          ))}
        </select>
